					...
					<div className='text-sm sm:flex sm:w-full justify-end'>
						{menuData.map((list: any, id: number) => (
							<NavLink
								...
								key={id}
								className='group relative text-blue-100 block sm:inline-block text-right p-3 text-md transition duration-500 ease-in-out  hover:text-blue-300  transform'
							>
								{list.title}
								...
								{list.children.length > 0 && (
									<div className='dropdown-menu absolute hidden text-gray-700 pt-1 group-hover:block left-0 shadow-lg mt-1 w-full sm:w-80'>
										{list.children.map((child: any, id: number) => {
											return (
												<Link
													to={child.route}
													key={id}
