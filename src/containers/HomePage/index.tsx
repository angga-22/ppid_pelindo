...
import { HomepageApi, NewsApi } from "../../api";
...
const Index = () => {
  ...
  const renderNews = () =>
    news?.map((n: any) => {
      let { img, title } = JSON.parse(n.data);
      return (
        <div className="overflow-hidden m-auto h-auto w-auto relative text-left pb-6">
          <img alt="" src={`${img}`} className="max-h-48 w-full object-cover" />
          <div className="h-36 flex flex-col justify-between w-full px-4 relative">
            ...
            <div className="">
              <NavLink
                to={`/news/${n.id}`}
                className="text-base text-blue-100 hover:text-blue-300"
              >
                Selengkapnya
              </NavLink>
            </div>
